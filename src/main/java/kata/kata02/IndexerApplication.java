package kata.kata02;

import java.lang.reflect.Array;

public class IndexerApplication {

    public static int INDUCEDNO = 10;
    public static final int[] FILLED_IN_ARRAY = {1,2,8,5,4};

    private ValueIndexFinder finder;

    public IndexerApplication(ValueIndexFinder finder) {
        this.finder = finder;

    }

    private void cokolwiek() {

    }
    public static void main(String[] args) {

       // IndexerApplication indexerApplication = new IndexerApplication();
       // finder.findIndex( value,  intsArray);

        //cokolwiek();

        //(do rozwiazania)



    }

    //java naming convention, konstruktory klas, wywolywanie metod (overload method->), sygnatura metody, przeciazanie metod, przeslanianie metod


}
